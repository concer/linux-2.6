...
/*
 * I don't know if any of this is needed, ask Tomek.
 */

/* read-write pulse length */
#define WR_MCH_FPGA_RW_PLEN		12

/* CS to read-write setup length */
#define WR_MCH_FPGA_CS2RW_SETUPLEN	2

/* read-write setup Length */
#define WR_MCH_FPGA_RW_SETUPLEN		((WR_MCH_FPGA_CS2RW_SETUPLEN) + \
					(WR_MCH_FPGA_SETUPLEN))
/* setup length */
#define WR_MCH_FPGA_SETUPLEN		2

/* hold length */
#define WR_MCH_FPGA_HOLDLEN		2

/* cycle length */
#define WR_MCH_FPGA_CYCLE_LEN		((WR_MCH_FPGA_RW_PLEN) +	\
					(WR_MCH_FPGA_C2RW_SETUPLEN) +	\
					(WR_MCH_FPGA_SETUPLEN) +	\
					2 * (WR_MCH_FPGA_HOLDLEN))

/*
 * enable PCK0 clock output at MCK/2
 * pck_enable(0, AT91_PMC_CSS_PLLA, AT91_PMC_PRES_2);
 */

/* Configure the EBI1 timing for 62.5 MHz refclk2 inside FPGA */
	at91_sys_write(AT91_SMC1_SETUP(0),
		AT91_SMC_NWESETUP_(WR_MCH_RW_SETUPLEN) |
		AT91_SMC_NCS_WRSETUP_(WR_MCH_FPGA_SETUPLEN) |
		AT91_SMC_NRDSETUP_(WR_MCH_RW_SETUPLEN) |
		AT91_SMC_NCS_RDSETUP_(WR_MCH_FPGA_SETUPLEN));

	at91_sys_write(AT91_SMC1_PULSE(0),
		AT91_SMC_NWEPULSE_(WR_MCH_FPGA_RW_PLEN) |
		AT91_SMC_NCS_WRPULSE_(WR_MCH_FPGA_RW_PLEN +
				2 * WR_MCH_FPGA_CS2RW_SETUPLEN) |
		AT91_SMC_NRDPULSE_(WR_MCH_FPGA_RW_PLEN) |
		AT91_SMC_NCS_RDPULSE_(WR_MCH_FPGA_RW_PLEN +
				2 * WR_MCH_FPGA_CS2RW_SETUPLEN));

	at91_sys_write(AT91_SMC1_CYCLE(0),
		AT91_SMC_NWECYCLE_(WR_MCH_FPGA_CYCLE_LEN) |
		AT91_SMC_NRDCYCLE_(WR_MCH_FPGA_CYCLE_LEN));
	...
	_writel(ptr + AT91_SMC1_SETUP(0), AT91_SMC_NWESETUP_(4) |
		AT91_SMC_NCS_WRSETUP_(2) |
		AT91_SMC_NRDSETUP_(4) |
		AT91_SMC_NCS_RDSETUP_(2));
	_writel(ptr + AT91_SMC1_PULSE(0), AT91_SMC_NWEPULSE_(4) |
		AT91_SMC_NCS_WRPULSE_(8) |
		AT91_SMC_NRDPULSE_(4) |
		AT91_SMC_NCS_RDPULSE_(8));
	_writel(ptr + AT91_SMC1_CYCLE(0), AT91_SMC_NWECYCLE_(12) |
		AT91_SMC_NRDCYCLE_(12));
	_writel(ptr + AT91_SMC1_MODE(0), AT91_SMC_DBW_32 |
		AT91_SMC_TDF_(0) |
		AT91_SMC_READMODE |
		AT91_SMC_WRITEMODE);
#endif
