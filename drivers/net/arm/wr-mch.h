 ...
 */
#define WR_NIC_CTL		0x0004
#define WR_NIC_CTL_TX_EN	BIT(5)	/* enable TX if there's data */
#define WR_NIC_CTL_RX_EN	BIT(4)	/* enable RX if there's free space */
#define WR_NIC_CTL_TX_PAUSE	BIT(3)	/* send pause frame immediately */
#define WR_NIC_CTL_RX_BAD_CRC	BIT(2)	/* accept frames with invalid CRC */
#define WR_NIC_CTL_RX_RUNT	BIT(1)	/* enable reception of 'runt' frames */
#define WR_NIC_FLOW_CTL_ENABLE	BIT(0)	/* enable flow control */
#define WR_NIC_CTL_MASK		0x003F

/*
 * NIC Interrupt Enable Register
 */
#define WR_NIC_IER		0x0008
#define WR_NIC_IER_TXI		BIT(4)	/* TX done */
#define WR_NIC_IER_RXI		BIT(3)	/* RX done */
#define WR_NIC_IER_RXPAUSE	BIT(2)	/* pause frame received */
#define WR_NIC_IER_TXERR	BIT(1)	/* TX error (on fabric i/f) */
#define WR_NIC_IER_RXERR	BIT(0)	/* RX error (on fabric or overflow) */
#define WR_NIC_IER_MASK		0x001F
/*
 * this is broken:
 * - if I receive a TX Interrupt, how do I know which TX has been sent?
 * - in NAPI polling, how do I know what remains to be done?
 * A table of descriptors, managed by the hardware, seems the only
 * sane solution.
 */
 ...
 */
#define WR_NIC_ISR		0x000C
#define WR_NIC_ISR_TXI		BIT(4)
#define WR_NIC_ISR_RXI		BIT(3)
#define WR_NIC_ISR_RXPAUSE	BIT(2)
#define WR_NIC_ISR_TXERR	BIT(1)
#define WR_NIC_ISR_RXERR	BIT(0)
...
#define WR_NIC_TX_DESC_START	0x0024
/* address of the last word of the last (newest) TX descriptor */
#define WR_NIC_TX_DESC_END	0x0028
...
#define WR_NIC_RX_DESC_START	0x002C
/* address of the last word of the last(newest) RX descriptor */
#define WR_NIC_RX_DESC_END	0x0030
 ...
 */
#define WR_NIC_TX_OFFSET	0x10000
#define WR_NIC_RX_OFFSET	0x20000

/*
 * NIC RX Pending
 * Set to 1 when there are pending packets to be handled
 * i.e. set to 0 when RX_DESC_START == RX_DESC_END
 */
#define WR_NIC_RX_PENDING	0x00034
